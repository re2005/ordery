...
Google Analytics 4 is currently hardcoded in `app/root.tsx` with measurement ID `G-GH291DY9MC`.

To change or remove it:

1. Open `app/root.tsx`.
2. Locate: `const GA_MEASUREMENT_ID = "G-GH291DY9MC";`
3. Replace with your new ID or delete the two `<script>` tags to disable GA.

Because it's hardcoded, no environment variable is required. If you later prefer an environment-based approach, reintroduce a loader that returns `process.env.GA_MEASUREMENT_ID` and conditionally render the scripts.
